...
void main()
{
    //fragColor = texture(stateTexture, varTexcoord.st, 0.0);
	gl_FragColor = gl_Color * texture2D(stateTexture, gl_TexCoord[0].xy);
	//gl_FragColor.rgb = color (texture2D (tex0, gl_TexCoord[0].xy).r);
}
